...
class FeeManController extends BaseController
{
    ...
    public function feeAdd($feeHead)
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($collectionType == 'halfYearly') {
                ...
                $applicableFrom = 'fH:' . $firstHalfApplicableFrom . ',sH:' . $secondHalfApplicableFrom;
                $applicableTill = 'fH:' . $firstHalfApplicableTill . ',sH:' . $seconHalfApplicableTill;
                ...
                $applicableFrom = 'Y:' . $yearApplicableFrom;
                $applicableTill = 'Y:' . $yearApplicableTill;
            }
    ...
    public function editFee($id = null)
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            $formUpdData = [
                ...
                'collectionType' => $collectionType = $this->request->getVar('collectype'),
                'totalAmount' => $totalAmount = $this->request->getVar('totalAmount'),
                'applicableFrom' => $applicableFrom = $this->request->getVar('applicableFrom'),
                'applicableTill' => $applicableTill = $this->request->getVar('applicableTill'),
            ];
            //   print_r($formUpdData);
    ...
    public function studentFeeAdd()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            $feeAmout = $this->request->getVar('feeAmount');
            $applicableTill = $this->request->getVar('applicableTill');
            ...
            if (isset($program)) {
               ...
               // echo '<hr>';
                ...
                for ($i = 0; $i < count($admno); $i++) {
                    for ($j = 0; $j < count($feeHead); $j++) {
                        $studentFeeData[] = [
                            ...
                            'fee_head' => $feeHead[$j],
                            'Amount' => $feeAmout[$j],
                            'dueDate' => $applicableTill[$j],
                            'accadamicYear' => $accadamicYearData
                        ];
                    }
                }
                ...
                foreach ($studentFeeData as $data) {
                    $key = $data['admissionNo'] . $data['fee_head'];
                    if (!isset($uniqueStudentFeeData[$key])) {
    ...
    public function studenntFeeDueFilter()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            $resultData['stdinfo'] = $stdFeeOpsModelObj->findData($yearOfAdmission, $accadamicYear, $program);
            if(!empty($resultData)){
